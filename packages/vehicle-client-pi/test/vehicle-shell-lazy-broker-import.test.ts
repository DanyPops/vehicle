 ...
 *
 * The fix: discoverAllVehicles() (now in vehicle-shell/state.ts -- vehicle-shell.ts itself is a
 * pure re-export composition root since the Vehicle Pass 1 SRP split) only ever imports
 * vehicle-shell-broker.ts's TYPES statically (fully erased, no runtime import emitted) and its
 * VALUE lazily, via a dynamic import() reached only when a consumer's own options.broker is
 * actually set. This test guards the invariant directly against the source text rather than
 * trying to reconstruct a real node_modules-nested install layout just to reproduce Node's own
 * restriction.
 */
describe("vehicle-shell/state.ts never statically (eagerly) imports vehicle-shell-broker.ts", () => {
	const source = readFileSync(resolve(import.meta.dir, "..", "src", "vehicle-shell", "state.ts"), "utf8");

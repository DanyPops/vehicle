/**
 * renderVehicleResult -- renders a completed (or in-progress) Vehicle tool call's own output,
 * driven by shape-probing (single/multi-array envelopes, plain-content narration, flat records)
 * when no projected vehicle.tool-details/v1 presentation or registered VehiclePresenter claims
 * it first. Split out of vehicle-render.ts's own bundled concerns (Vehicle Pass 1 SRP audit
 * finding #6) -- the result-rendering half, cleanly separable from the call-rendering half.
 */

import { extractVehicleContent, type VehicleOperationDescriptor } from "@danypops/vehicle-core";
...
import {
	type GenericVehiclePresentation,
	parseGenericVehiclePresentation,
	type VehiclePresentationField,
...
export type RenderResultContext = Parameters<NonNullable<ToolDefinition["renderResult"]>>[3];
 ...
 * domain-agnostic transform as call-rendering's own humanizeOperationName, applied to a flat
 * record's own field names instead of an operation name. */

/**
 * hitl-ask-prompt-layout.ts -- row layout for the searchable single-select list used by
 * requestPiAskPrompt (hitl-ask-prompt.ts). Wraps long option titles/descriptions to a target
...
export interface PiAskPromptOption {
	title: string;
	description?: string;
}
...
type ListItem =
	| { type: "option"; option: PiAskPromptOption }
	| { type: "comment-toggle"; option: PiAskPromptOption }
	| { type: "freeform"; option: PiAskPromptOption };
